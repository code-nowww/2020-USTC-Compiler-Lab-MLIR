...
static mlir::LogicalResult verify(ConvValidOp op) {
...
/// Infer the output shape of the ConvOp, this is required by the shape inference
/// interface.
void FillFullOp::inferShapes() { 
  //getResult().setType(getOperand(0).getType()); 
  auto targetTy = getOperand(0).getType().cast<RankedTensorType>();
  auto kernelTy = getOperand(1).getType().cast<RankedTensorType>();
  SmallVector<int64_t, 2> dims;
  dims.push_back(targetTy.getShape().vec()[0] + 2*kernelTy.getShape().vec()[0] - 2);
  dims.push_back(targetTy.getShape().vec()[1] + 2*kernelTy.getShape().vec()[1] - 2);
  //TODO:how to extend???
  //SmallVector<int64_t, 2> dims(llvm::reverse(targetTy.getShape()));
  getResult().setType(RankedTensorType::get(dims, targetTy.getElementType()));
...
static mlir::LogicalResult verify(FillFullOp op) {
  auto targetType = op.getOperand(0).getType().dyn_cast<RankedTensorType>();
  auto kernelType = op.getOperand(1).getType().dyn_cast<RankedTensorType>();
  auto resultType = op.getType().dyn_cast<RankedTensorType>();
  if (!targetType || !kernelType || !resultType)
  ...
  auto kernelShape = kernelType.getShape().vec();
  auto resultShape = resultType.getShape().vec();
   if (resultShape[0]!= inputShape[0] + 2*kernelShape[0] - 2) {
    return op.emitError()
           << "error in the ConvFull OP's result shape[0]";
  }
  if (resultShape[1]!= inputShape[1] + 2*kernelShape[1] - 2) {
    return op.emitError()
           << "error in the ConvFull OP's result shape[1]";
  }
  if (targetType.getElementType() != kernelType.getElementType()) {
    return op.emitError()
           << "error in the ConvFull OP's operands type, should have same type";
  }
  return mlir::success();
}
...
static mlir::LogicalResult verify(FillSomeOp op) {
  auto targetType = op.getOperand(0).getType().dyn_cast<RankedTensorType>();
  auto kernelType = op.getOperand(1).getType().dyn_cast<RankedTensorType>();
  auto resultType = op.getType().dyn_cast<RankedTensorType>();
  if (!targetType || !kernelType || !resultType)
  ...
  if(kernelShape[0] != kernelShape[1]){
    return op.emitError()
           << "error in ConvSome: kernel expect to be square";
  }
  if(kernelShape[0]%2 != 1){
    return op.emitError()
           << "error in ConvSome: width expect to be even";
  }
  if (resultShape[0]!= inputShape[0] + kernelShape[0] - 1) {
    return op.emitError()
           << "error in the ConvSome OP's result shape[0]";
  }
  if (resultShape[1]!= inputShape[1] + kernelShape[1] - 1) {
    return op.emitError()
           << "error in the ConvSome OP's result shape[1]";
  }
  if (targetType.getElementType() != kernelType.getElementType()) {
    return op.emitError()
           << "error in the ConvSome OP's operands type, should have same type";
  }
  return mlir::success();
}
...
static mlir::LogicalResult verify(ConvSomeOp op) {
  auto targetType = op.getOperand(0).getType().dyn_cast<RankedTensorType>();
  auto kernelType = op.getOperand(1).getType().dyn_cast<RankedTensorType>();
  auto resultType = op.getType().dyn_cast<RankedTensorType>();
  if (!targetType || !kernelType || !resultType)
